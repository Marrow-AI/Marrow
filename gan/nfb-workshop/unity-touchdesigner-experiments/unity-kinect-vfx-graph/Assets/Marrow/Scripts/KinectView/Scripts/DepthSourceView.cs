...
public class DepthSourceView : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (ViewMode == DepthViewMode.SeparateSourceReaders)
        {
            ...
            gameObject.GetComponent<Renderer>().material.SetTexture("_MainTex", _ColorManager.GetColorTexture());

            RefreshData(_DepthManager.GetData(), _ColorManager.ColorWidth, _ColorManager.ColorHeight);
        }
        else
        {
            ...
            gameObject.GetComponent<Renderer>().material.SetTexture("_MainTex", _MultiManager.GetColorTexture());
